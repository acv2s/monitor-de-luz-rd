 ...
 * propia factura. Si nadie ha vinculado su chat todavía, se manda a los
 * destinatarios generales para no dejar al dueño sin sus avisos de siempre.
 */
async function avisarContrato(cid: number | null, texto: string): Promise<boolean> {
  const chats = cid != null ? await chatsDelContrato(cid) : [];
  if (!chats.length) return sendTelegram(texto, { dashboardButton: true });
...
async function avisarFoto(cid: number | null, url: string, caption: string): Promise<void> {
  const chats = cid != null ? await chatsDelContrato(cid) : [];
  if (!chats.length) { await sendTelegramPhoto(url, caption); return; }
...
async function procesarContrato(c: Contrato, log: string[]): Promise<boolean> {
  ...
  try {
    ...
    await alert(c.id, c.nic ?? '—', 'error_corrida', `error:${c.id}:${hoy}`, 'warning',
      `La corrida diaria falló para ${etiqueta}: ${msg}`, propio).catch(() => {});
    await cerrar(false);
    return false;
  }
}
